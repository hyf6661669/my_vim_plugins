...
autocmd BufWrite *.vh   call UpdateLastModifyTime()
autocmd BufWrite *.svh  call UpdateLastModifyTime()
autocmd BufWrite *.sva  call UpdateLastModifyTime()




"Search instance / module definition 
...
function s:GetNegedge(signeg) 
	return s:GetPosedge(a:signeg) + s:clk_period/2
endfunction 

function s:SigLastClkIsHigh(lnum,posedge,negedge) 
	let ret = 0
...
function s:SigCurClkIsHigh(lnum,posedge,negedge) 
	let ret = 0
    let line = getline(a:lnum - 1)
    if line[a:negedge-1] =~ '-'
        let ret = 1
...
function s:SigNextClkIsHigh(lnum,posedge,negedge) 
	let ret = 0
    let line = getline(a:lnum - 1)
...
function s:BusCurClkHaveChg(lnum,posedge,negedge) 
	let ret = 0
...
function AddHeader() 
	...
	call append( 2,  "// Author             : Yifei He")
	call append( 3,  "// How to Contact     : hyf_sysu@qq.com")
	call append( 4,  "// Created Time       : ".strftime("%Y-%m-%d %H:%M:%S"))
	call append( 5,  "// Last Modified Time : ")
	call append( 6,  "// ================================================================================================================")
	call append( 7,  "// Description:")
	call append( 8,  "// ================================================================================================================")
	call append( 9,  "")
	call append(10,  "// ================================================================================================================")
	call append(11,  "// Copyright (C) ".strftime("%Y"). " HYF. ALL RIGHTS RESERVED.")
	call append(12,  "// ================================================================================================================")
    " Write the LICENSE information Below
endfunction 
...
function UpdateLastModifyTime() 
    " Line number is related with the code in AddHeader
	let line = getline(6)
	if line =~ '// Last Modified Time'
		call setline(6, "// Last Modified Time : ".strftime("%Y-%m-%d %H:%M:%S"))
	endif
endfunction 
...
"WireDef2PortConn definition 

function WireDef2PortConn() 
	let lnum = line(".")
	let line = getline(lnum)
    if line =~ '^\s*\/\/' || line =~ '^\s*$'
        return 0
...
function s:SearchComment(lnum,curcol) 
	" 1 found, 0 no found
...
function s:ShowInstWrapper(mode,lnum,word) 
...
function s:GetInstLineNum(lnum) 
...
function s:GetInstNameAdvance(lnum) 
"get rtl.module_name
...
function s:GetInstPortDir(inst,port) 
...
function s:SkipCommentLine(mode,line_idx) 
...
function s:SkipCommentLine_array(mode,lines,line_idx) 
	"mode: 0 - code search dir is up to down, 1 - is down to up
	"return no comment line index
	"must run before comment line
	let in_comment_pair = 0
...
function s:SkipPairCommentLine_array(mode,lines,line_idx) 
	"mode: 0 - code search dir is up to down, 1 - is down to up
	"return no comment line index
	"must run before comment line
	let in_comment_pair = 0
...
"===============================================================
function s:UpdateBuf(new_lines,goto_lnum) 
...
function s:GetSeqIO(lines, io_seq_dict) 
...
function s:ExtendIoFromLine(dict,in_line,init_seq) 
...
function s:ExtendIoFromLine2(dict,in_line,init_seq) 
	"line must be input or output define line
    let io_dir = matchstr(a:in_line,'\w\+')
	"let line = substitute(a:in_line,'^\s*\(\<input\>\|\<output\>\|\<inout\>\)\s*','','')
	let line = substitute(a:in_line,'^\s*' . s:VlogTypePorts . '\s*','','')
...
function s:ConvertSeqIO(io_seq_dict,io_dict) 
...
function s:ConvertIO(io_dict,io_seq_dict) 
...
function s:ExtendUsrdefFromLine(dict,in_line,init_seq) 
	"line must be input or output define line
...
function s:ExtendFromSide(dict,side,type) 
	let signal_name = ''
	let width = ''
	let has_defined = ''
	let init_seq = ''
...
function s:ExtendInstWireFromLine(dict,in_line,inst_io) 
	let line = a:in_line
...
function s:GetSpecAutoParaLine(state) 
...
function s:GetAutoParas(autoparaline) 
...
function s:GetAllSignals(alldefs,allparas) 
	let allsignals = {}
...
function s:UpdateLinkDict(link_dict,allparas,key,in_dict) 
" link_dict:
"	{
"		signal_name :
"		{
...
function s:GroupLinkDict(link_dict) 
...
function s:MvLinkDict(link_dict,key0,key1) 
...
function s:UpdateDefine(unresolved,link_dict,signals) 
...
function s:DivSignals(signals,io_wire,usr_def,ff_reg,comb_reg,wire,inst_wire) 
...
function s:ConvertSignals(signals,io_wire) 
	let max_len = []
...
function s:GetPortMaxLen(signals) 
	let max_len = []
	let prefix_max_len = s:autoinst_prefix_max_len
	let suffix_max_len = s:autoinst_suffix_max_len
...
"===============================================================
function KillAutoInst(kill_all) 
   let line_index = 1
   let goto_lnum = -1
...
function AutoInstUpdate(kill_all) 
    "======================================================================================
    "Function:
...
function AutoInstUpdateOrder(kill_all) 
    "======================================================================================
    "Function:
...
function AppendPortDriectionToInst(kill_all) 
    "======================================================================================
    "Function:
...
function s:oTreeNode.GetTopNode(tree) 
    let node = a:tree
    while 1
        if(node.layer == 1)
            break
...
function s:oTreeNode.SearchChildNodeByLnum(tree,lnum) 
...
function s:oTreeNode.SearchChildNodeByInstname2(tree,inst2) 
    for node in a:tree.children
...
function s:oTreeNode.SearchNodeByLnum(tree,lnum) 
    return self.SearchChildNodeByLnum(self.GetTopNode(a:tree),a:lnum)
endfunction 
function s:oTreeNode.CreateRtlTree(tree) 
    if a:tree.filename == ''
...
function s:oTreeNode.DrawRtlTree(prefix,tree) 
...
function s:oTreeNode.RenderTree() 
...
function s:oTreeNode.TreeLog(log) 
    if s:rtl_tree_is_open == 0
        return
    endif
...
function s:oTreeNode.TreeLogInstFullPath() 
    let fullpath = ''
    let node = s:current_node
    while 1
...
function s:setupTreeSyntaxHighlighting() 
...
function s:CloseRtlTree() 
    execute bufwinnr(t:NERDTreeBufName) . " wincmd w"
    close
    let t:RtlBufName = s:GetInstFileName(s:current_node.instname)
    execute bufwinnr(t:RtlBufName) . " wincmd w"
endfunction 
function s:OpenRtlTreeLog() 
